  ...
  <div class="pro-page page-animate subscription-pro">
    <div class="wrap subscription-wrap">
      ...
      <section class="plans-grid-new">
        <article v-for="plan in plans" :key="plan.id" class="subscription-plan" :class="{featured:plan.id==='premium'}">
          <div v-if="plan.id==='premium'" class="popular-ribbon">MOST POPULAR</div>
          <div class="plan-head"><span class="plan-tier">{{ plan.id==='premium' ? 'Premium workspace' : 'Lite workspace' }}</span><span class="plan-mark">{{ plan.id==='premium' ? 'P' : 'L' }}</span></div>
          <h3>{{ plan.name }}</h3>
          <p class="plan-description">{{ plan.description }}</p>
          <div class="plan-billing"><div><strong>{{ priceLabel(plan) }}</strong><span v-if="plan.monthlyConfigured">monthly billing</span><span v-else>Billing setup</span></div><span class="stripe-chip">Stripe</span></div>
          <ul class="plan-features"><li>Football odds & market context</li><li>Live scores and match centre</li><li>Transparent prediction history</li><li>Dropping Odds intelligence</li><li v-if="plan.id==='premium'">Premium prediction content</li><li v-if="plan.id==='premium'">Deeper research workspace</li></ul>
          <div class="plan-actions"><button class="btn" :disabled="busy||!isConfigured(plan)" @click="checkout(plan.id,'month')">{{ !auth.isLoggedIn ? 'Create a member account' : isConfigured(plan) ? 'Subscribe monthly' : 'Coming soon' }}</button><button class="ghost" v-if="isConfigured(plan,'year')" @click="checkout(plan.id,'year')">Yearly billing</button></div>
        </article>
      </section>
...
function priceLabel(p:any){const value=p.monthlyPrice??p.monthlyAmount??p.price; if(typeof value==='number'&&value>0)return `${p.currency||'USD'} ${value}`; return isConfigured(p)?'Available':'Not configured';}
