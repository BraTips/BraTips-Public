...
function priceLabel(p:any,i:'month'|'year'='month'){const fallback=p.id==='premium'?(i==='year'?199:19.99):(i==='year'?99:9.99);const value=i==='year'?(p.yearlyPrice??p.yearlyAmount??fallback):(p.monthlyPrice??p.monthlyAmount??p.price??fallback);return `${p.currency||'USD'} ${Number(value).toFixed(2)}`;}
