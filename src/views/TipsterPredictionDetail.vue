      ...
      <template v-else>
        ...
        <section v-if="prediction.locked" class="premium-access-shell">
          <div class="premium-access-hero">
            ...
            <button type="button" class="ghost premium-share-button" @click="sharePrediction">Share pick</button>
            <p class="premium-note">Already a member? <RouterLink to="/login">Sign in</RouterLink> to continue.</p>
          </div>
        </section>

        <template v-else>
          <section class="tipster-prediction-hero"><div><span class="eyebrow">Tipster prediction</span><h1>{{ prediction.prediction }}</h1><p>{{ fixture }}</p></div><div class="tipster-prediction-result"><span>{{ prediction.status || 'published' }}</span><strong>{{ formatOdds(prediction.odds) }}</strong><small>Quoted odds</small><button type="button" class="tipster-detail-share-button" @click="sharePrediction">Share</button></div></section>
