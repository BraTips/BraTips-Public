  ...
  <div class="history-page page-animate"><div class="wrap">
    ...
    <section class="history-list-card">
      ...
      <article v-for="p in filteredItems" :key="p._id" class="history-row">
        <div class="history-date">{{formatDate(p.resultAt||p.createdAt)}}</div>
    ...
    <section class="history-summary">
      <div><span>Settled tips</span><strong>{{stats.total||0}}</strong></div><div><span>Win rate</span><strong>{{Number(stats.winRate||0).toFixed(1)}}%</strong></div><div><span>Total profit</span><strong :class="Number(stats.profit||0)>=0?'positive':'negative'">{{formatProfit(stats.profit)}}</strong></div><div><span>Automatic settlement</span><strong>Live</strong></div>
    </section>
  </div></div>
</template>
...
import {formatProfit,formatOdds,formatDate} from '../utils/formatters';
