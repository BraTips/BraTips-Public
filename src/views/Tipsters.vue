    ...
    <div class="wrap">
      ...
      <section class="tip-overview">
        <div class="tip-overview-stats">
          ...
          <strong class="big-profit">{{ formatProfit(overallProfit) }}</strong><span class="profit-label">TOTAL TRACKED PROFIT</span>
      ...
      <div v-else-if="filtered.length" class="tip-card-grid">
        <RouterLink v-for="(t,i) in filtered" :key="t._id" :to="`/tipsters/${encodeURIComponent(t.username)}`" class="tip-pro-card">
          ...
          <div class="tip-main-kpi"><div><span>Profit</span><strong :class="Number(t.profit||0)>=0?'positive':'negative'">{{formatProfit(t.profit)}}</strong></div><div><span>ROI</span><strong :class="Number(t.roi||0)>=0?'positive':'negative'">{{Number(t.roi||0).toFixed(1)}}%</strong></div></div>
...
import {formatProfit,formatShortDate} from '../utils/formatters';
