    ...
    <div class="wrap">
      ...
      <div v-else-if="filtered.length" class="tip-card-grid">
        <RouterLink v-for="(t,i) in filtered" :key="t._id" :to="`/tipsters/${encodeURIComponent(t.username)}`" class="tip-pro-card">
          ...
          <div class="tip-card-head"><div class="tip-photo"> <img v-if="t.profilePhoto" :src="t.profilePhoto" :alt="t.username"/><span v-else>{{t.username?.[0]?.toUpperCase()}}</span></div><div class="tip-card-name"><b>{{t.username}}</b><small>{{t.country || 'Football tipster'}}</small><div class="mini-form"><span v-for="(s,j) in streak(t)" :key="j" :class="s==='W'?'w':'l'">{{s}}</span></div></div><span class="rank-number">#{{i+1}}</span></div>
...
const streak=(t:any)=>{const n=Math.min(5,Math.max(0,Number(t.currentStreak||0)));return Array.from({length:5},(_,i)=>i<n?'W':'L')};
